...
import static com.aliyun.oss.common.utils.LoggerUtils.getLogger;
...
public class Mimetypes {
    ...
    public synchronized static Mimetypes getInstance() {
        ...
        if (is != null) {
            getLogger().debug("Loading mime types from file in the classpath: mime.types");
            
            try {
                mimetypes.loadMimetypes(is);
            } catch (IOException e) {
                getLogger().error("Failed to load mime types from file in the classpath: mime.types", e);
            } finally {
                try { 
                	is.close(); 
            ...
            getLogger().warn("Unable to find 'mime.types' file in classpath");
        }
        return mimetypes;
    }
